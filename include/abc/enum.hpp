...
#define ABC_ENUM(ENUM_NAME, ...)                                                                             \
    ...
    static const std::array<abc::string, static_cast<size_t>(ENUM_NAME::COUNT)>& __GET_##ENUM_NAME##_names() \
    {                                                                                                        \
...
#define ABC_ENUM_STRING_IMPL(SCOPE, ENUM_NAME)                                        \
    namespace abc {                                                                   \
    namespace detail {                                                                \
    template <> struct to_string_impl<SCOPE::ENUM_NAME> {                             \
        static abc::string impl(const SCOPE::ENUM_NAME& enumType)                     \
        {                                                                             \
            if (enumType == SCOPE::ENUM_NAME::COUNT) {                                \
                return "COUNT";                                                       \
            }                                                                         \
