...
namespace detail {
//////////////////////////////////////////////////////////////////////////
template <typename... TInnerErrors> class inner_error;
...
public:
    ...
    template <typename TErrorOrInner = TError> inline const abc::optional<TErrorOrInner>& get() const
    {
        return m_optError;
    }
    inline const abc::string message() const
    {
        return m_optError.has_value() ? m_optError->message() : inner_error<TInnerErrors...>::message();
    }
    ...
    {
        return m_optError.has_value() && other.m_optError.has_value() && m_optError.get() && other.m_optError.get();
    }
    ...
    template <typename TInnerErrorCode> bool operator==(const TInnerErrorCode& errorCode) const
    {
        return m_optError.has_value() && m_optError.get() == errorCode;
    }
};

template <typename TError> class inner_error<TError> {
    abc::optional<TError> m_optError;
...
public:
    inner_error() = default;
    inline inner_error(const inner_error<TError>& other)
        : m_optError(other.m_optError)
    {
    }
    inline inner_error(const error_t& err)
        : m_optError(err)
    {
    }
    inline inner_error(const typename TError::code_t& errorCode)
    ...
    template <typename TInnerOrError = TError> inline const abc::optional<TInnerOrError>& get() const
    {
        return m_optError;
    }
    inline const abc::string message() const { return m_optError.has_value() ? m_optError->message() : abc::string(); }
    inline const abc::string message_with_inner() const
    {
...
*/
template <typename TErrorCode, typename... TInnerErrors> class error {
...
public:
    explicit inline error(const code_t i_code)
        : m_code(i_code)
    {
    }
    inline error(const code_t& i_code, const char* i_message)
        : m_code(i_code)
        , m_message(i_message)
    {
    }
    ...
    inline error(const error<code_t, TInnerErrors...>& other)
        ...
        , m_innerErrors(other.m_innerErrors)
    {
    }
    ...
    template <typename TInnerError> inline bool has_inner() const
    {
        return m_innerErrors.template get<TInnerError>().has_value();
    }
    template <typename TInnerError> inline TInnerError inner() const
    {
...
template <typename TErrorCode> class error<TErrorCode, void> {
public:
    using code_t = TErrorCode;

public:
    error(TErrorCode i_code)
        : m_code(i_code)
    {
    }
    error(TErrorCode i_code, const char* i_message)
        : m_code(i_code)
        , m_message(i_message)
    {
    }
    error(TErrorCode i_code, const abc::string& i_message)
        : m_code(i_code)
        , m_message(i_message)
    {
    }
...
template <typename TPayload, typename TError = error<generic_error_type>> class result : abc::noncopyable {
public:
...
public:
    ...
    inline result(const payload_t& res)
        : m_optPayload(res)
    {
    }
    inline result(payload_t&& res)
        : m_optPayload(std::move(res))
    {
    }

    inline result(const error_t& err)
        : m_optError(err)
    {
    }
    inline result(error_t&& err)
        : m_optError(std::move(err))
    {
    }
    inline result(const error_code_t& error_code)
        : m_optError(error_t(error_code))
    {
    }
    ...
    inline bool operator!=(const error_code_t& i_errorCode) const { return !operator==(i_errorCode); }
...
template <typename TError> class result<void, TError> : abc::noncopyable {
public:
    using error_t      = TError;
    using error_code_t = typename error_t::code_t;
...
public:
    ...
    result(const error_t& err)
        : m_optError(err)
    {
    }
    result(const error_code_t& error_code)
        : m_optError(error_t(error_code))
    {
    }
