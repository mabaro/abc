...
#pragma message("core.hpp must be included before pointer.h")
#endif

#ifdef ABC_TESTING_ENABLED
#pragma message("ABC_TESTING is enabled. Some protected members are now public for testing purposes.")
#endif
...
template <typename T> class lendable;
...
template <typename T> class unique_pointer_internal;
template <typename T> class lent_pointer_internal;
template <typename T> class lendable_internal;

template <typename T> class unique_pointer_promoter;
template <typename T> class lent_pointer_promoter;

//////////////////////////////////////////////////////////////////////////
// UNIQUE POINTERS
//////////////////////////////////////////////////////////////////////////

template <typename T> class unique_pointer_internal : public abc::noncopyable {
...
public:
    ...
    inline void reset(T* pointer = nullptr)
    {
        ...
        if (m_pointer != pointer) {
            ...
            if (m_controlBlock) {
                ...
                if (references == 0) {   // otherwise let it dangle and notify
                    delete m_controlBlock;
                } else {
                    ABC_FAIL_RELEASE("UNIQUE_PTR_DANGLING: unique_ptr deleted with {} references", references);
                }
    ...
    template <typename T2> inline void reset(T2* pointer = nullptr)
    {
...
public:
    ...
    inline explicit unique_pointer_internal(bool) = delete;   // avoid implicit cast from nullptr on some compilers
    ...
    inline explicit unique_pointer_internal(T* pointer)
        : m_pointer(pointer)
        , m_controlBlock(pointer ? new pointer_control_block() : nullptr)
    {
    }
    template <typename T2>
    ...
    template <typename T2> inline unique_pointer_internal<T> operator=(const std::nullptr_t&)
    {
        ABC_ASSERT((m_pointer == nullptr && m_controlBlock == nullptr) || m_controlBlock->has_references(),
    ...
    template <typename T2> inline this_t& operator=(unique_pointer_internal<T2>&& other)
    {
        static_assert((std::is_class<T>::value == false || std::is_base_of<T, T2>::value),
            "Only upcast conversions are implicit. Use static_unique_cast.");
...
protected:   // lent_pointer_checked API
    ...
    {
        ABC_ASSERT(m_controlBlock, "Accessing a dangling lent_pointer, it's unique_point has been destroyed");
...
protected:
    template <typename T2> friend class unique_pointer_internal;
    template <typename T2> friend class lent_pointer_internal;

    template <typename T2> friend class detail::lent_pointer_promoter;
    template <typename T2> friend class detail::unique_pointer_promoter;

protected:   // comparisons
    template <typename T1> friend bool operator==(const unique_pointer_internal<T1>& lhs, std::nullptr_t);
    template <typename T1> friend bool operator==(std::nullptr_t, const unique_pointer_internal<T1>& rhs);
    template <typename T1, typename T2>
    friend bool operator==(const unique_pointer_internal<T1>& lhs, const unique_pointer_internal<T2>& rhs);
    template <typename T1, typename T2> friend bool operator==(const unique_pointer_internal<T1>& lhs, const T2* rhs);
    template <typename T1, typename T2> friend bool operator==(const T1* lhs, const unique_pointer_internal<T2>& rhs);
    template <typename T1, typename T2> friend bool operator==(const unique_pointer_internal<T1>& lhs, const T2 rhs);
    template <typename T1, typename T2> friend bool operator==(const T1 lhs, const unique_pointer_internal<T2>& rhs);
...
inline bool
operator==(const unique_pointer_internal<T1>& lhs, std::nullptr_t)
{
    return lhs.m_pointer == nullptr;
}
template <typename T1>
inline bool
operator==(std::nullptr_t, const unique_pointer_internal<T1>& rhs)
{
    return nullptr == rhs.m_pointer;
}
template <typename T1, typename T2>
inline bool
operator==(const unique_pointer_internal<T1>& lhs, const unique_pointer_internal<T2>& rhs)
{
    return lhs.m_pointer == rhs.m_pointer;
}
template <typename T1, typename T2>
inline bool
operator==(const unique_pointer_internal<T1>& lhs, const T2* rhs)
{
    return lhs.m_pointer == rhs;
}
template <typename T1, typename T2>
inline bool
operator==(const T1* lhs, const unique_pointer_internal<T2>& rhs)
{
...
template <typename T> class unique_ref : public detail::unique_pointer_internal<T> {
    using parent_t = detail::unique_pointer_internal<T>;

    inline unique_ref(parent_t&& other)
        : parent_t(std::move(other))
    {
    }
    template <typename T2>
    inline unique_ref(detail::unique_pointer_internal<T2>&& other)
        : parent_t(std::move(other))
    {
    }
...
public:
    ...
    template <typename T2> unique_ref(const unique_ptr<T2>&) = delete;
    template <typename T2> unique_ref(const unique_ref<T2>&) = delete;

    template <typename T2>
    unique_ref(T2* pointer)
        : parent_t(static_cast<T*>(pointer))
    {
    ...
    template <typename T2>
    unique_ref(unique_ref<T2>&& other)
        : parent_t(std::move(other))
    {
        static_assert(std::is_const<T>::value || !std::is_const<T2>::value,
            "Trying to assign a const object to a non-const object");
...
template <typename T> class unique_ptr : public detail::unique_pointer_internal<T> {
    using parent_t = detail::unique_pointer_internal<T>;
...
public:
    ...
    inline explicit unique_ptr(T* pointer)
        : parent_t(pointer)
    {
    }
    inline unique_ptr(unique_ptr<T>&& other)
        : parent_t(std::move(other))
    {
    }
    inline unique_ptr(unique_ref<T>&& other)
        : parent_t(std::move(other))
    {
    }
    ...
    template <typename T2> unique_ptr<T>& operator=(unique_ptr<T2>&& other)
    {
        parent_t::operator=(std::move(other));
        return *this;
    }
    ...
    template <typename T2> unique_ptr<T>& operator=(unique_ref<T2>&& other)
    {
        parent_t::operator=(std::move(other));
        return *this;
    }
...
public:
    ...
    lendable_internal(Args&&... args)
        ...
        , m_controlBlock(new pointer_control_block())
    {
    }
    ...
    inline T* get_raw_ptr() const { return const_cast<typename std::remove_const<T*>::type>(&m_object); }
...
protected:
    template <typename T2> friend class lent_pointer_internal;
    template <typename T2> friend class lendable_helper;
};

//////////////////////////////////////////////////////////////////////////

template <typename T> class lendable_helper;
...
template <typename T> class lendable : public detail::lendable_internal<T> {
protected:
...
public:
    ...
    template <typename... Args>
    inline lendable(Args&&... args)
        : parent_t(std::forward<Args>(args)...)
    {
    }

    inline lendable(const this_t& other)
        : parent_t::m_object(other->m_object)
    {
    }
    inline lendable(this_t&& other)
        : parent_t(std::forward(other))
    {
    }

    inline lendable(const T& object)
        : parent_t::m_object(object)
    {
    }
    inline lendable(T&& object)
        : parent_t(std::forward<T>(object))
    {
    }
...
protected:
    template <typename U> friend class detail::lendable_helper;
    template <typename U> friend class detail::lent_pointer_internal;
};
...
protected:
    ...
    explicit lent_pointer_internal(const unique_ref<T2>& uniqueReference)
        : m_pointer(uniqueReference.m_pointer)
        , m_controlBlock(uniqueReference.m_controlBlock)
    {
        static_assert((std::is_class<T>::value == false || std::is_base_of<T, T2>::value),
...
protected:
    template <typename U> friend class lent_pointer_internal;
    template <typename U> friend class lent_pointer_promoter;
    template <typename U> friend class unique_pointer_internal;
    template <typename U> friend class lendable_internal;
    template <typename U> friend class lendable_helper;

protected:
    template <typename T1> friend bool operator==(const lent_pointer_internal<T1>& lhs, std::nullptr_t);
    template <typename T1> friend bool operator==(std::nullptr_t, const lent_pointer_internal<T1>& rhs);
    template <typename T1, typename T2>
    friend bool operator==(const lent_pointer_internal<T1>& lhs, const detail::lent_pointer_internal<T2>& rhs);
    template <typename T1, typename T2> friend bool operator==(const lent_pointer_internal<T1>& lhs, const T2* rhs);
    template <typename T1, typename T2> friend bool operator==(const T1* lhs, const lent_pointer_internal<T2>& rhs);
    template <typename T1, typename T2>
    friend bool operator==(const unique_pointer_internal<T1>& lhs, const lent_pointer_internal<T2>& rhs);
    template <typename T1, typename T2>
    friend bool operator==(const lent_pointer_internal<T1>& lhs, const unique_pointer_internal<T2>& rhs);
...
template <typename T1>
inline bool
operator==(const lent_pointer_internal<T1>& lhs, std::nullptr_t)
{
    return lhs.m_pointer == nullptr;
}
template <typename T1>
inline bool
operator==(std::nullptr_t, const lent_pointer_internal<T1>& rhs)
{
    return nullptr == rhs.m_pointer;
}
template <typename T1, typename T2>
inline bool
operator==(const lent_pointer_internal<T1>& lhs, const lent_pointer_internal<T2>& rhs)
{
    return lhs.m_pointer == rhs.m_pointer;
}
template <typename T1, typename T2>
inline bool
operator==(const lent_pointer_internal<T1>& lhs, const T2* rhs)
{
    return lhs.m_pointer == rhs;
}
template <typename T1, typename T2>
inline bool
operator==(const T1* lhs, const lent_pointer_internal<T2>& rhs)
{
    return lhs == rhs.m_pointer;
}
template <typename T1, typename T2>
inline bool
operator==(const unique_pointer_internal<T1>& lhs, const lent_pointer_internal<T2>& rhs)
{
    return lhs.m_pointer == rhs.m_pointer;
}
template <typename T1, typename T2>
inline bool
operator==(const lent_pointer_internal<T1>& lhs, const unique_pointer_internal<T2>& rhs)
{
    return lhs.m_pointer == rhs.m_pointer;
}
...
template <typename T> class lent_ref : public detail::lent_pointer_internal<T> {
...
public:
    ...
    lent_ref(const lent_ref<T>& other)
        : parent_t(other)
    {
    }
    template <typename T2>
    lent_ref(const lent_ref<T2>& other)
        : parent_t(other)
    {
    }
    template <typename T2>
    lent_ref(const unique_ref<T2>& uniqueReference)
        : parent_t(uniqueReference)
    {
        static_assert(std::is_const<T>::value || !std::is_const<T2>::value,
            "Trying to assign a const object to a non-const object");
    }
...
protected:
    template <typename T2> friend class detail::lent_pointer_promoter;
    template <typename T2> friend class detail::lendable_helper;

    template <class T2> friend class lendable;
    template <class T2> friend class detail::lendable_internal;
};

template <typename T> class lent_ptr : public detail::lent_pointer_internal<T> {
    using parent_t = detail::lent_pointer_internal<T>;
...
public:
    ...
    explicit lent_ptr(std::nullptr_t)
        : parent_t()
    {
    }
    lent_ptr(const lent_ptr<T>& other)
        : parent_t(other)
    {
    }
    template <typename T2>
    lent_ptr(const lent_ptr<T2>& other)
        : parent_t(other)
    {
    }
    template <typename T2>
    lent_ptr(const lent_ref<T2>& other)
        : parent_t(other)
    {
    }
    template <typename T2>
    lent_ptr(const unique_ref<T2>& uniqueReference)
        : parent_t(uniqueReference)
    {
        static_assert(std::is_const<T>::value || !std::is_const<T2>::value,
            "Trying to assign a const object to a non-const object");
    ...
    template <typename T2>
    lent_ptr(const unique_ptr<T2>& uniqueReference)
        : parent_t(uniqueReference)
    {
        static_assert(std::is_const<T>::value || !std::is_const<T2>::value,
            "Trying to assign a const object to a non-const object");
...
template <typename T> class lent_pointer_promoter {
public:
    template <typename T2> static inline lent_ref<T> promote(const lent_ptr<T2>& other)
    {
        ABC_ASSERT(other);
...
template <typename T>
inline unique_ref<T>
promote_to_ref(unique_ptr<T>&& pointer)
{
    ABC_ASSERT(pointer);
    if (pointer) {
...
template <typename T>
inline lent_ref<T>
promote_to_ref(const lent_ptr<T>& pointer)
{
    ABC_ASSERT(pointer != nullptr);
    if (pointer) {
...
template <typename T>
inline lent_ref<T>
promote_to_ref(const lent_ref<T>& pointer)
{
    static_assert(detail::always_fail<T>, "Cannot promote to reference a reference");
    return pointer;
}
...
template <bool, class L, class R> struct conditional_type_selector {
    typedef R type;
};

template <class L, class R> struct conditional_type_selector<true, L, R> {
    typedef L type;
};
...
public:
    template <typename T2>
    static lent_ref<typename std::enable_if<!std::is_const<T2>::value || std::is_const<T>::value, T>::type> cast(
        const lendable<T2>& object)
    {
        ...
        //										   const
        //std::remove_const<T*>::type, T*>::type>(&i_lendable.m_object), i_lendable.m_control_block));
...
template <typename T>
lent_ptr<T>
lend(const unique_ptr<T>& unique)
{
    return lent_ptr<T>(unique);
}
template <typename T>
lent_ref<T>
lend(const unique_ref<T>& unique)
{
    return lent_ref<T>(unique);
}
template <typename T>
lent_ref<T>
lend(const lendable<T>& object)
{
