...
public:
    ...
    void print_summary(const std::vector<abc::string>& tagFilter = std::vector<abc::string>()) const
    {
        ...
        const auto timeUnitsFunc = [](const abc::chrono::duration& duration) {
            ...
            {
                return abc::format(
                    "{} s", std::chrono::duration_cast<abc::chrono::secondsf>(duration).count());
            }
            else if (duration >= abc::chrono::milliseconds(1))
            {
                return abc::format(
                    "{} ms",
                    std::chrono::duration_cast<abc::chrono::millisecondsf>(duration).count());
            }
            else
            {
                return abc::format(
                    "{} us",
                    std::chrono::duration_cast<abc::chrono::microsecondsf>(duration).count());
            }
        };
        ...
        const auto processSample = [&timeUnitsFunc](const ProfilingData& data) {
            ...
            if (data.samples > 1)
            {
                if (data.mt_lockedTime > ProfilingData::duration(0))
                {
                    ...
                    std::cout << abc::format("{} : avg({})lckd({}) min/max({}/{})#[{}]", data.tag,
                                             timeUnitsFunc(avgTimeMT), timeUnitsFunc(lockedTime),
                                             timeUnitsFunc(data.minDuration),
                                             timeUnitsFunc(data.maxDuration), data.samples)
                              << std::endl;
                }
                else
                {
                    std::cout << abc::format("{} : avg({}) min/max({}/{})#[{}]", data.tag,
                                             timeUnitsFunc(avgTime),
                                             timeUnitsFunc(data.minDuration),
                                             timeUnitsFunc(data.maxDuration), data.samples)
                              << std::endl;
                }
            ...
            else
            {
                ...
                {
                    std::cout << abc::format("{} : {}", data.tag, timeUnitsFunc(avgTime))
                              << std::endl;
                }
            }
        };
        ...
        else
        {
            ...
            {
                auto it =
                    std::find_if(m_samples.begin(), m_samples.end(),
    ...
    void declare(const abc::string& tag, bool reset = false)
    {
        ...
        else
        {
            ...
            {
                ABC_LOG_DEBUG(
                    "Call to profiler::declare({}) failed because the tag is already registered",
                    tag);
            }
        }
    }
...
protected:
    ...
    {
        sample_container_iterator_t it =
            std::find_if(m_samples.begin(), m_samples.end(),
                         [&tag](const ProfilingData& data) { return data.tag == tag; });
        if (it != m_samples.end())
        {
...
protected:
    ...
    struct ProfilingData
    {
        ...
        ProfilingData(const abc::string& i_tag) : tag(i_tag) {}
        ProfilingData(const abc::string& i_tag, const time_point& i_t0) : tag(i_tag), t0(i_t0) {}
...
///////////////////////////////////////////////////////////////////////////////
#define PROFILE_SECTION(TAG, CODE_BLOCK)                 \
    do                                                   \
    {                                                    \
